...
export function useFormulario() {
  ...
  const save = async (input: FormularioInput): Promise<string> => {
    ...
    try {
      ...
      // Sincronizar a Supabase si es un cierre completo o un estimado disponible
      if (formulario.estado === 'completo' || formulario.fase === 'estimado') {
        try {
  ...
  const update = async (formulario: Formulario): Promise<void> => {
    ...
    try {
      ...
      // Actualizar en Supabase si es un cierre completo o un estimado disponible
      if (formulario.estado === 'completo' || formulario.fase === 'estimado') {
        try {
          // Si el formulario es estimado, no esperamos block-sync porque los otros dos tipos pueden no existir aún.
